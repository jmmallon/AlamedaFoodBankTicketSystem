...
app.get('/tickets/not-done-from-last-12-hours', (req, res) => {
  ...
  const query = `
    SELECT * FROM tickets 
    WHERE time >= ? AND done = 0 AND  positionInLine > 0
  `;
...
app.get('/tickets/latest', (req, res) => {
  ...
  // const query = 'SELECT * FROM tickets WHERE time >= ? AND positionInLine > 0 ORDER BY positionInLine DESC LIMIT 1';
  const query = 'SELECT * FROM tickets WHERE time >= ? ORDER BY id DESC LIMIT 1';
...
// GET endpoint to retrieve the ticket with the biggest "positionInLine" where "done" is false from the last 12 hours
app.get('/tickets/latestnum', (req, res) => {
  const twelveHoursAgo = new Date(Date.now() - 12 * 60 * 60 * 1000).toISOString();
...
app.post('/tickets', async (req, res) => {
  ...
  const { firstName, lastName, scheduleAppointment, secondFamily, firstTimeVisitor, additionalNotes, done, scheduleAppointmentTime } = req.body;
  ...
  try {
   ...
   const positionInLine = (secondFamily) ? 0 : maxPositionInLine + 1;
